...
icmp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
     ...
     */
        icmp_output(ICMP_TYPE_ECHO_REPLY, hdr->icmp_code, hdr->dep, (uint8_t *)(hdr + 1), len - sizeof(*hdr),
                    iface->unicast, iphdr->src);
        break;
    default:
        /* ignore */
        break;
