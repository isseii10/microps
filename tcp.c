...
static ssize_t tcp_output(struct tcp_pcb *pcb, uint8_t flg, const uint8_t *data, size_t len);
...
static ssize_t
tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, const uint8_t *data, size_t len,
...
tcp_retrans_emit(void *arg, struct queue_entry *_entry)
{
    ...
    if (timercmp(&now, &timeout, >)) {
        ...
        tcp_output_segment(entry->seq, pcb->rcv.nxt, entry->flg, pcb->rcv.wnd, (uint8_t *)(entry + 1), entry->len,
...
static void
tcp_segment_arrives(struct seg_info *seg, uint8_t flags, const uint8_t *data, size_t len, ip_endp_t local,
                    ip_endp_t remote)
{
        ...
        */
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (pcb->mode == TCP_PCB_MODE_SOCKET) {
                ...
                debugf("allocate PCB for new connection, desc=%d, state=%s", tcp_pcb_desc(new_pcb),
                       tcp_state_ntoa(new_pcb->state));
                new_pcb->parent = pcb;
                pcb = new_pcb;
            }
            pcb->local = local;
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (pcb->snd.una > pcb->iss) {
                ...
                /* ignore: If there are other controls or text in the segment, 
                   queue then for processing after the ESTABLISHED state has been reached
                */
                return;
            }
        }
...
tcp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    if (src.addr == IP_ADDR_BROADCAST || src.addr == iface->broadcast || dst.addr == IP_ADDR_BROADCAST ||
        dst.addr == iface->broadcast) {
...
tcp_cmd_open(ip_endp_t local, ip_endp_t remote, int active)
{
    ...
    debugf("mode=%s, local=%s, remote=%s", active ? "active" : "passive", ip_endp_ntop(local, ep1, sizeof(ep1)),
...
AGAIN:
    ...
    debugf("success, local=%s, remote=%s", ip_endp_ntop(pcb->local, ep1, sizeof(ep1)),
...
tcp_cmd_connect(int desc, ip_endp_t remote)
{
    ...
    if (local.addr == IP_ADDR_ANY) {
        iface = ip_route_get_iface(remote.addr);
        if (!iface) {
            errorf("iface not fount that can reach remote address, addr=%s",
                   ip_addr_ntop(remote.addr, addr, sizeof(addr)));
            lock_release(&lock);
            return -1;
        }
...
tcp_cmd_accept(int desc, ip_endp_t *remote)
{
    ...
    debugf("success: desc=%d, local=%s, remote=%s", new_desc, ip_endp_ntop(new_pcb->local, ep1, sizeof(ep1)),
