...
udp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    debugf("%s => %s, len=%zu, dev=%s", ip_endp_ntop(src, endp1, sizeof(endp1)),
           ip_endp_ntop(dst, endp2, sizeof(endp2)), len, NET_IFACE(iface)->dev->name);
    udp_print(data, len);
    lock_acquire(&lock);
...
udp_output(ip_endp_t src, ip_endp_t dst, const uint8_t *data, size_t len)
{
    ...
    debugf("%s => %s, len=%zu", ip_endp_ntop(src, endp1, sizeof(endp1)), ip_endp_ntop(dst, endp2, sizeof(endp2)),
           total);
    udp_print(buf, total);
