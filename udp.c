...
static void udp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface) {
  ...
  dst.addr = iphdr->dst;
  dst.port = hdr->dst;
  debugf("%s => %s, len=%zu, dev=%s", ip_endp_ntop(src, endp1, sizeof(endp1)), ip_endp_ntop(dst, endp2, sizeof(endp2)),
  ...
  if (!pcb) {
    ...
    icmp_output(ICMP_TYPE_DEST_UNREACH, ICMP_CODE_PORT_UNREACH, 0, (uint8_t *)iphdr, iphdrlen + 8, iface->unicast,
                iphdr->src);
    return;
  }
...
static ssize_t udp_output(ip_endp_t src, ip_endp_t dst, const uint8_t *data, size_t len) {
  ...
  debugf("%s => %s, len=%zu", ip_endp_ntop(src, endp1, sizeof(endp1)), ip_endp_ntop(dst, endp2, sizeof(endp2)), total);
  udp_print(buf, total);
...
int udp_cmd_bind(int desc, ip_endp_t local) {
  ...
  if (exist) {
    errorf("already in use, desc=%d, want=%s, exist=%s", desc, ip_endp_ntop(local, endp1, sizeof(endp1)),
