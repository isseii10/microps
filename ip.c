...
static struct ip_route *ip_route_add(ip_addr_t network, ip_addr_t netmask, ip_addr_t nexthop, struct ip_iface *iface) {
  ...
  if (nexthop != IP_ADDR_ANY) {
    infof("%s/%s via %s dev %s src %s", ip_addr_ntop(network, addr1, sizeof(addr1)),
          ip_addr_ntop(netmask, addr2, sizeof(addr2)), ip_addr_ntop(nexthop, addr3, sizeof(addr3)),
          NET_IFACE(iface)->dev->name, ip_addr_ntop(iface->unicast, addr4, sizeof(addr4)));
  } else {
    infof("%s/%s dev %s src %s", ip_addr_ntop(network, addr1, sizeof(addr1)),
          ip_addr_ntop(netmask, addr2, sizeof(addr2)), NET_IFACE(iface)->dev->name,
...
static ssize_t ip_build_packet(uint8_t protocol, const uint8_t *data, size_t len, uint16_t id, uint16_t offset,
                               ip_addr_t src, ip_addr_t dst, uint8_t *buf, size_t size) {
  uint16_t hlen, total;
  struct ip_hdr *hdr;
  ...
  hdr->sum = 0; // cksum16()やる前に0で初期化しないとcksumがおかしくなる
  hdr->src = src;
  hdr->dst = dst;
